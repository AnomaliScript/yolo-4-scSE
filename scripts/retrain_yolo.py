...
def continue_training():
    ...
    WEIGHTS = Path('runs/vertebra_detector3/weights/best.pt')
    ...
    results = model.train(
        ...
        project='runs',
        cache='ram',       # Cache images in your 32GB RAM (MUCH faster!)
        amp=True,          # Automatic mixed precision (saves memory)
        name='vertebra_detector4',  # New name to avoid overwriting
    ...
    print(f"{'='*70}")
    print(f"New model saved to: runs/vertebra_detector4/weights/best.pt")
    print(f"{'='*70}")
